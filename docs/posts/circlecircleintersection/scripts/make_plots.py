...
def make_cap_images():
    ...
    circle2_translated1 = Circle([3.1, 0], 0.8)
    circle2_translated2 = Circle([5.6, 0], 0.8)
    ...
    # second circle
    circle2_translated1.plot(ax, fill=False, edgecolor=circle2_color, linewidth=2)
    angles = get_angles_from_intersection(circle2, intersect_pts)
    draw_arc_area(ax, circle2_translated1.mean, circle2_translated1.radius, angles[0], angles[1], color=circle2_color, hatch="/", alpha=0.4)
    
    # third circle
    circle2_translated2.plot(ax, fill=False, edgecolor=circle2_color, linewidth=2)
    
    points = np.zeros((3, 2))
    points[0, :] = circle2_translated2.mean
    points[1, :] = circle2_translated2.mean + circle2_translated2.radius * np.array([np.cos(angles[0]), np.sin(angles[0])])
    points[2, :] = circle2_translated2.mean + circle2_translated2.radius * np.array([np.cos(angles[1]), np.sin(angles[1])])
    triangle = mpatches.Polygon(points, closed=True, color=circle2_color, hatch="/", alpha=0.4)
    ax.add_patch(triangle)


    # math symbols
    plt.text(1.4, -0.2, r"$=$", c=circle2_color, fontsize=40)
    plt.text(4.0, -0.2, r"$-$", c=circle2_color, fontsize=40)

    min_x = np.minimum(circle2.mean[0]-circle2.radius, circle2_translated2.mean[0]-circle2_translated2.radius)
    max_x = np.maximum(circle2.mean[1]+circle2.radius, circle2_translated2.mean[0]+circle2_translated2.radius)
    plt.xlim(min_x - 0.1, max_x + 0.1)
    plt.ylim([-1.2, 1.2])
    ax.set_aspect('equal', 'box')
...
def make_cap2_images():
    fig1 = plt.figure(dpi=120)
    ax = plt.gca()

    circle1 = Circle([-0.5, 0], 1)
    circle2 = Circle([-0.1, 0], 0.8)
    ...
    # drawing caps
    draw_cap_from_intersect(ax, circle1, intersect_pts, color=circle1_color, hatch="/", alpha=0.4)
    draw_cap_from_intersect(ax, circle2, intersect_pts, smaller_area=False, color=circle2_color, hatch="/", alpha=0.4)
    ...
    # labels
    plt.text(0.25, 0., r"$A_1$", c=circle1_color, fontsize=17)
    plt.text(-0.4, 0., r"$A_2$", c=circle2_color, fontsize=17)
    ...
    fig1.savefig('../images/caps2.png', bbox_inches='tight')


def make_right_angle_image():
    fig = plt.figure(dpi=120)
    ax = plt.gca()
    circle1 = Circle([-0.5, 0], 1)
    circle2 = Circle([0.6, 0], 0.8)
    ...
    # center line
    plt.plot([circle1.mean[0], circle2.mean[0]], [circle1.mean[1], circle2.mean[1]], c="xkcd:red", linewidth=2)
    ...
    delta = 0.1
    pts = np.tile(center_pt, [3, 1])
    pts[0] = pts[0] + np.array([0, delta])
    pts[1] = pts[1] + np.array([-delta, delta])
    pts[2] = pts[2] + np.array([-delta, 0])
    plt.plot(pts[:, 0], pts[:, 1], color="xkcd:red")

    # drawing triangles
    tri1_pts = np.zeros((3, 2))
    tri1_pts[0, :] = center_pt
    tri1_pts[1, :] = intersect_pts[1]
    tri1_pts[2, :] = circle1.mean
    tri1 = mpatches.Polygon(tri1_pts, closed=True, color=circle1_color, alpha=0.5)
    ax.add_patch(tri1)

    tri2_pts = np.zeros((3, 2))
    tri2_pts[0, :] = center_pt
    tri2_pts[1, :] = intersect_pts[1]
    tri2_pts[2, :] = circle2.mean
    tri2 = mpatches.Polygon(tri2_pts, closed=True, color=circle2_color, alpha=0.5)
    ax.add_patch(tri2)
    ...
    fig.savefig('../images/right_angle.png', bbox_inches='tight')
